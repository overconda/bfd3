...
class SBF_API {
    ...
    public function checkBaseUnlockQuiz($route_id, $base_id, $base_no, $oauth_user_id, $quiz_id, $answer) {
        ...
        if ($quiz['correct_answer'] == $answer) {
            ...
            addScore(1 /*action: correct quiz*/, $user_level, $oauth_user_id, $route_id, $base_id );
            addScore(2 /*action: Unlocked*/, $user_level, $oauth_user_id, $route_id, $base_id);
    ...
    public function setBaseUser($base_id, $oauth_user_id, $score) {
        ...
        if ($challenge == 'win') {
            ...
            //// added by Over Sep 10,2017
            $user_level = getUserLevel($oauth_user_id);
            addScore(3 /*action: Be Guardian*/, $user_level, $oauth_user_id, $route_id, $base_id);
        } else {
            $time = new DateTime();
            $minutes_to_add = 3;
            $time->add(new DateInterval('PT' . $minutes_to_add . 'M'));
    ...
    ***********************************/
    ...
    public function addScore($action_id, $level_id, $oauth_user_id, $route_id=0 , $base_id=0){
      ...
      if ($result->num_rows) {
          ...
          $sql .= " (oauth_user_id, action_id, level_num, score, txtReason, route_id, base_id, cdate) ";
    ...
    public function getUserScore($oauth_user_id){
...
if ($_POST['method'] == "get_route_base_user") {
    ...
    header('Content-Type: application/json');
    echo(json_encode($response));
} else if ($_POST['method'] == "get_user_score") {
    $response = $sbf_api->getUserScore($_POST['oauth_user_id']);
    header('Content-Type: application/json');
    echo(json_encode($response));
}
